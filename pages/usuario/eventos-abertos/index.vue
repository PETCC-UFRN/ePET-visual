        ...
        <div v-if="eventos.length > 0">          
          ...
          >
            <template v-slot:cell(actions)="row">
              ...
              <b-button
                  @click.prevent="inscrever(row.item.idEvento, row.item.valor, row.item)"
...
export default {
  ...
  methods: {
    ...
    inscrever(idEvento, valor, evento) {

        let inscricao_inicio = moment(evento.d_inscricao).format('DD/MM'); 
        let inscricao_fim = moment(evento.d_inscricao_fim).format('DD/MM'); 

        let evento_inicio = moment(evento.d_evento_inicio).format('DD/MM'); 
        let evento_fim = moment(evento.d_evento_fim).format('DD/MM'); 

        let valorEvento = new Intl
